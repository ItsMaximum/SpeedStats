...
|---|---|---|
| **crawl** (`crawl.py`) | Lists all series and games, then fetches every game's data and every leaderboard page. Requests go through a pool of proxies (`proxy_pool.py`, one IP each, ~100 req/min per IP, automatic back-off on 429). Rows are appended as they arrive; a checkpoint per game makes a crashed crawl resumable. | `data/work/crawl-<version>.duckdb` (raw tables: `raw_runs`, `raw_players`, `raw_games`, ...) |
